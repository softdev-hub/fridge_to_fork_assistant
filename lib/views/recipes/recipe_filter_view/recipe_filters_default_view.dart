...
class RecipeFiltersDefaultView extends StatefulWidget {
  ...
  @override
  State<RecipeFiltersDefaultView> createState() =>
      _RecipeFiltersDefaultViewState();
}

class _RecipeFiltersDefaultViewState extends State<RecipeFiltersDefaultView> {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final maxWidth = constraints.maxWidth >= 768 ? 480.0 : 412.0;
  ...
  Widget _chipOption(String label) {
    final bool isMeal = ['Sáng', 'Trưa', 'Tối', 'Bữa phụ'].contains(label);
    final isSelected = isMeal
        ? _selectedMeals.contains(label)
        : _selectedCuisines.contains(label);

    return GestureDetector(
      onTap: () {
        setState(() {
          if (isMeal) {
            if (!_selectedMeals.remove(label)) {
              _selectedMeals.add(label);
            }
          } else {
            if (!_selectedCuisines.remove(label)) {
